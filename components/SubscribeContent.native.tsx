...
import { View, Text, TouchableOpacity, StyleSheet, SafeAreaView, ScrollView, Image, Alert, Platform } from 'react-native'
...
export default function SubscribeContent() {
  ...
  const fetchSubscriptionParams = async (plan: 'monthly' | 'yearly') => {
    ...
    PaymentLogger.log('CLIENT_FETCH_START', `Fetching subscription params for client session ${clientSessionId}`, {
      plan,
    ...
    PaymentLogger.log('CLIENT_RESPONSE_RECEIVED', `API response received for client session ${clientSessionId}`, {
    ...
    if (!response.ok) {
      ...
      PaymentLogger.error('CLIENT_API_ERROR', `API error for client session ${clientSessionId}`, errorData)
      throw new Error(errorData.error || 'Failed to fetch subscription params')
    }
    
    const responseData = await response.json()
    PaymentLogger.logSafeData('CLIENT_FETCH_SUCCESS', `Subscription params fetched for client session ${clientSessionId}`, responseData)
    
    const { paymentIntent, ephemeralKey, customer, subscription } = responseData
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      PaymentLogger.log('CLIENT_SUBSCRIBE_START', `Subscribe process started for client session ${clientSessionId}`, {
        selectedPlan,
        platform: Platform.OS,
        deviceInfo: {
          version: Platform.Version,
      ...
      const { paymentIntent, ephemeralKey, customer, subscription } = await fetchSubscriptionParams(selectedPlan)
      
      PaymentLogger.log('CLIENT_SUBSCRIPTION_DATA_RECEIVED', `Subscription data received for client session ${clientSessionId}`, {
        customerId: customer,
        subscriptionId: subscription
      })

      // Initialize payment sheet with Apple Pay support
      PaymentLogger.log('CLIENT_PAYMENT_SHEET_INIT', `Initializing payment sheet for client session ${clientSessionId}`, {
        ...
        subscriptionLabel: selectedPlan === 'monthly' ? 'Monthly Subscription' : 'Yearly Pass',
      ...
      const { error } = await initPaymentSheet({
        ...
        applePay: {
          ...
          cartItems: [
            {
              label: selectedPlan === 'monthly' ? 'Monthly Subscription' : 'Yearly Pass',
              amount: selectedPlan === 'monthly' ? '4.99' : '49.99',
      ...
      if (error) {
        PaymentLogger.error('CLIENT_PAYMENT_SHEET_INIT_ERROR', `Payment sheet init failed for client session ${clientSessionId}`, error)
        throw error
      }
      
      PaymentLogger.success('CLIENT_PAYMENT_SHEET_INIT_SUCCESS', `Payment sheet initialized for client session ${clientSessionId}`)

      // Present the payment sheet
      PaymentLogger.log('CLIENT_PAYMENT_SHEET_PRESENT', `Presenting payment sheet for client session ${clientSessionId}`)
      
      const { error: presentError } = await presentPaymentSheet()

      if (presentError) {
        if (presentError.code === 'Canceled') {
          PaymentLogger.warn('CLIENT_PAYMENT_CANCELLED', `Payment cancelled by user for client session ${clientSessionId}`, {
            code: presentError.code
          })
        } else {
          PaymentLogger.error('CLIENT_PAYMENT_ERROR', `Payment error for client session ${clientSessionId}`, presentError)
          throw presentError
        }
      } else {
        PaymentLogger.success('CLIENT_PAYMENT_SUCCESS', `Payment successful for client session ${clientSessionId}`, {
          selectedPlan,
          subscriptionId: subscription
        })
      ...
      PaymentLogger.error('CLIENT_SUBSCRIBE_ERROR', `Subscribe process failed for client session ${clientSessionId}`, error)
      
      Alert.alert('Payment Error', error.message || 'Something went wrong. Please try again.')
    } finally {
      setLoading(false)
      PaymentLogger.log('CLIENT_SUBSCRIBE_END', `Subscribe process completed for client session ${clientSessionId}`, {
        loading: false
      })
    }
  }
