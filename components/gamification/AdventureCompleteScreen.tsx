...
const smartWrapTitle = (title: string): string => {
  ...
  if (words.length >= 4) {
    ...
    // Only split if first part is reasonable length (10-20 chars)
    if (firstPart.length >= 10 && firstPart.length <= 20) {
      return `${firstPart}\n${secondPart}`;
    }
  }
        ...
        <View style={styles.titleContainer}>
          {hasMultipleLines && titleLine1 ? (
            <>
              ...
              <Text style={[styles.titleLine2, { marginTop: -SCREEN_HEIGHT * 0.045 }]} numberOfLines={2}>{titleLine2}</Text>
            </>
          ) : (
...
const styles = StyleSheet.create({
  ...
  titleLine2: {
    ...
    textAlign: 'center',
    lineHeight: SCREEN_WIDTH * 0.11, // Slightly larger for better spacing (~42px)
  },
